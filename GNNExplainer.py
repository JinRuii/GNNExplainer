...
import numpy as np
import os
from torch_geometric.nn import SAGEConv, GNNExplainer  # In version 2.0.4, import GNNExplainer from torch_geometric.nn
from torch_geometric.data import Data
from sklearn.preprocessing import MinMaxScaler

# Define the TwoLayerSAGEModel class (a two-layer GraphSAGE model)
class TwoLayerSAGEModel(torch.nn.Module):
    def __init__(self, num_features, sage_hidden_units1, sage_hidden_units2, num_classes, dropout):
        super(TwoLayerSAGEModel, self).__init__()
...
hyperparams = {
    ...
    'learning_rate': 0.001,
    'dropout': 0.5,
    'sage_hidden_units1': 128,
    'sage_hidden_units2': 128,
}
...
# 1. Load the model
output_dir = "./model_output"  # Replace with the actual output directory path
model_save_path = os.path.join(output_dir, 'trained_model.pth')
num_classes = len(label_mapping)
num_features = 20  # Number of features (based on your data, should be 20)

# Create a model instance
model = TwoLayerSAGEModel(
...
# Load labels and map them to integers
...
explainer = GNNExplainer(model, epochs=600)  # In version 2.0.4, no need to specify return_type
...
# 4. Save the results for node and edge contributions
# Save node feature importance
